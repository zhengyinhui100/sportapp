...
function(Dat,Util,Str,AbstractModule,Message,Messages,Common,MessageView){
	
	var Conversation=AbstractModule.derive({
		title           : '私信',
	...
	function fInit(){
		var me=this;
		var oReceiver=me.model;
		oReceiver._talking=true;
		oReceiver.set('newMsgNum',0);
		var nReceiverId=oReceiver.get('id');
		var sId;
		if(gUid<nReceiverId){
			sId=gUid+'-'+nReceiverId;
		}else{
			sId=nReceiverId+'-'+gUid;
		}
		var oConversation=me.conversation=oReceiver.get('conversation');
		if(!oConversation.id){
			oConversation.id=sId;
		}
		me.add([
			Common.getHeader(oReceiver.get('nickname')),
			{
				xtype:'Panel',
	...
	function fSendMsg(){
		...
		if(oInput.valid()){
			...
			var oNewMsgInfo={
				senderId:gUid,
				receiverId:me.model.get('id'),
				createTime:Dat.now(true),
				content:sValue
			};
			var oNewMessage=new Message(oNewMsgInfo);
			oNewMessage.save();
			//添加到会话集合中
